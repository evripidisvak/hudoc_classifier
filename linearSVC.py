...
def fetch_cases():
    try:
        ...
        cursor.execute("SELECT normalized_article, case_id FROM articles WHERE normalized_article != '';")
...
def objective(trial, X_train, y_train):
    """Define Optuna's optimization function for Linear SVC with pruning."""
    params = {
            "C": trial.suggest_float("C", 0.01, 10.0, log=True),
            "tol": trial.suggest_float("tol", 1e-5, 1e-3, log=True),
            "max_iter": trial.suggest_int("max_iter", 1000, 5000), "dual": False,
            # Set to False which is better for cases with n_samples > n_features
    ...
    clf = LinearSVC(**params, random_state=42)

    # Wrap with CalibratedClassifierCV to get probability predictions
    calibrated_svc = CalibratedClassifierCV(clf, cv=3, method='sigmoid')

    # Use cross validation scoring for more robust evaluation
    from sklearn.model_selection import cross_val_score
    scores = cross_val_score(calibrated_svc, X_train, y_target, cv=3, scoring='neg_log_loss')

    # Explicitly force garbage collection
    gc.collect()

    # Return the negative mean log loss (to minimize)
    return -scores.mean()
...
def evaluate_models(models, X_val, y_val):
    ...
    for idx, article in enumerate(ARTICLES_COLUMNS):
        ...
        # For probability scores (if needed)
        y_prob = model.predict_proba(X_val)[:, 1]

        score_dict = classification_report(y_val[:, idx].toarray().ravel(), y_pred, digits=4, zero_division=numpy.nan,
                output_dict=True, )
...
def evaluate_model(model, article, X_val, y_val, idx):
    ...
    score_dict = classification_report(y_val[:, idx].toarray().ravel(), y_pred, digits=4, zero_division=numpy.nan,
            output_dict=True, )
    score = classification_report(y_val[:, idx].toarray().ravel(), y_pred, digits=4, zero_division=numpy.nan)
    print(f"Article {article}:\n {score}")
...
def check_class_distribution(y):
    """Check if a class has enough samples for 3-fold CV."""
    positive_samples = numpy.sum(y)
    return positive_samples >= 3
...
if __name__ == "__main__":
    ...
    pivot_df = pd.crosstab(df_articles["case_id"], df_articles["article"], values=1,  # Set value to 1 for all matches
    ...
    train_df, val_df = train_test_split(cases_to_articles_df, test_size=0.2, random_state=42)
    ...
    if not os.path.exists(os.path.join(MODEL_DIR, HYPERPARAMETERS)):
        ...
        X_train_opt, y_train_opt = create_optimization_subset(X_train, y_train, SAMPLE_SIZE_FOR_OPTUNA)
        study = optuna.create_study(direction="minimize",  # Minimize log loss
                pruner=optuna.pruners.MedianPruner(n_startup_trials=5, n_warmup_steps=3), )
        study.optimize(lambda trial: objective(trial, X_train_opt, y_train_opt), n_trials=20, show_progress_bar=True,
                gc_after_trial=True, )
    ...
    # Train Linear SVC Models with Best Parameters
    models = {}
    scores = {}
    evaluation_results = {}

    # Remove max_iter from best_hyperparams if it exists
    max_iter = best_hyperparams.pop("max_iter", 2000)
    ...
    for idx, article in enumerate(ARTICLES_COLUMNS):
        ...
        if not numpy.any(y_train_article == 1):
            print(f"No positive samples for {article}, using DummyClassifier.")
            model = DummyClassifier(0)
        else:
            base_clf = LinearSVC(**best_hyperparams, random_state=42, dual=False)

            if check_class_distribution(y_train_article):
        ...
        print("\nEvaluating model...")
        evaluation_results[article] = evaluate_model(model, article, X_val, y_val, idx)

        print("Saving model...")
        model_path = os.path.join(MODEL_DIR, f"linear_svc_{article}.pkl")
        with open(model_path, "wb") as f:
            pickle.dump(model, f)

    with open("linear_svc.json", "w") as f:
